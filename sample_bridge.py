...
def main():
    ...
    sampler = MVSECSampler(sampler=SingleMVSECSampler(scenario=scenario, split=training_data_split, is_training=True), 
                           batch_size=batch_size, drop_last=False)
    test_sampler = MVSECSampler(sampler=SingleMVSECSampler(scenario=scenario, split=training_data_split, is_training=False), 
                                batch_size=batch_size, drop_last=False)
    ...
    for i, (event_data, image_data, depth_data, disp_data, index_data) in enumerate(dataloader):
        ...
        # print('disp_data min/max after normalization:', disp_data.min().item(), disp_data.max().item())
        ...
        x0 = interpolate(x0, mode='bicubic', size=(resize_size, resize_size))
        y0 = interpolate(y0, mode='bicubic', size=(resize_size, resize_size))
            ...
            for image_idx in range(num_display):
                ...
                if x0 is not None:
                    vutils.save_image(
                        ...
                        f"{images_folder}/x_{image_idx}.png", nrow=1)
                ...
                for j in range(num_rep_images):
                    ...
                    vutils.save_image(
                        ...
                        f"{images_folder}/sample_{image_idx}.png", nrow=1)
                    # continue
                    vutils.save_image(
                        y0[image_idx:image_idx+1, j].unsqueeze(1) / 2 + 0.5,
                        f"{images_folder}/y_{image_idx}.png", nrow=1)
                    ...
                    for intermediate_x0_idx in range(len(pred_x0)):
                        ...
                        vutils.save_image(
                            ...
                            f"{images_folder}/intermediate_x0_at_time_idx{intermediate_x0_idx}.png", nrow=1)
          
        if i == 0 and dist.get_rank() == 0:
            if x0 is not None:
                vutils.save_image(
                    x0[:num_display, 0].unsqueeze(1) / 2 + 0.5,
                    f"{images_folder}/full_x_{i}.png",
                    nrow=int(np.sqrt(num_display)),
                )
            for j in range(num_rep_images):
                vutils.save_image(
                    sample.permute(0, 3, 1, 2)[:num_display, j].unsqueeze(1).float() / 255,
                    f"{images_folder}/full_sample_{i}_idx{j}.png",
                    nrow=int(np.sqrt(num_display)),
                )
                
                vutils.save_image(
                    y0[:num_display, j].unsqueeze(1) / 2 + 0.5,
                    f"{images_folder}/full_y_{i}_idx{j}.png",
                    nrow=int(np.sqrt(num_display)),
                )
                ...
                for intermediate_x0_idx in range(len(pred_x0)):
                    ...
                    vutils.save_image(
                        intermediate_x0.permute(0, 3, 1, 2).float() / 255,
                        f"{images_folder}/full_intermediate_x0_at_time_idx{intermediate_x0_idx}_RepIdx{j}.png", 
                        nrow=int(np.sqrt(num_display)),
                    )
        ...
        pred = interpolate(sample.permute(0, 3, 1, 2).float()[:half_batch, 0].unsqueeze(1), size=original_img_size, mode='bicubic')
        # print(sample.permute(0, 3, 1, 2).float()[:half_batch, 0].unsqueeze(1).shape, pred.shape, org_disp_data.shape)
        # exit()
        # org_disp_data = interpolate(org_disp_data.float(), size=original_img_size, mode='bicubic')
